...
//        modCount++;
//
//        // overflow-conscious code 这个容量比实际空间大
...
//        modCount++;
//        E oldValue = elementData(index);
//        // 比如一个长度为10的数组,移除第5个元素 index=5
//        // numMoved 为4 即拷贝的长度为4
//        // 就从源数组第6个位置开始copy 从第5个位置开始覆盖 覆盖4个长度.
...
//
//    /**
//     * 本质上是求交集 Retains only the elements in this list that are contained in the
...
//        try {
//            //这一步主要将不需要替换的数据前移,同时w记录的值右边的元素都是需要舍弃的对象
//            for (; r < size; r++) {
...
//            // 这里一定成立??
//            if (r != size) {
//                //r正常应为size-1 这里不会进行copy
//                System.arraycopy(elementData, r, elementData, w, size - r);
//                //w+=1
//                w += size - r;
//            }
//            //如果w==size  说明上面每次循环都进去了  集合没有修改
//            if (w != size) {
//                // clear to let GC do its work
